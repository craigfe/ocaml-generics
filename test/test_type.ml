...
let (my_record : my_record Type.t), Optic.[ name; flag; count ] =
  let open Type in
...
let record () =
  ...
  r |> Fmt.pr "%a\n" Type.(pp my_record);
  r |> Optic.view name |> Fmt.pr "%a\n" Type.(pp string);
  r |> Optic.view flag |> Fmt.pr "%a\n" Type.(pp bool);
  r |> Optic.view count |> Fmt.pr "%a\n" Type.(pp int)
...
let (subrecord_a : subrecord_a Type.t), Optic.[ a_foo; a_bar ] =
  let open Type in
...
let (subrecord_b : subrecord_b Type.t), Optic.[ b_foo ] =
  let open Type in
  record "subrecord_b" (fun b_foo -> { b_foo })
  |+ field "b_foo" (list subrecord_a) (fun s -> s.b_foo)
  |> sealr_lens

let (nested_record : nested_record Type.t), Optic.[ a; b; c ] =
  let open Type in
